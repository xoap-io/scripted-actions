<#
.SYNOPSIS
    Start a stopped Google Cloud VM instance using the gcloud CLI.

.DESCRIPTION
    This script starts a stopped Google Compute Engine VM instance using
    `gcloud compute instances start`. If ProjectId or Zone are not provided
    they are resolved from the active gcloud configuration. Supports async
    mode to return immediately without waiting for the operation to finish.
...
.PARAMETER Async
    Return immediately without waiting for the start operation to complete.

.EXAMPLE
    .\gce-cli-start-vm.ps1 -InstanceName "web-server-01"

    Start a VM using the project and zone from the active gcloud config.
...
.EXAMPLE
    ...
    Start a VM in the specified project and zone without waiting for completion.
...
param(
    ...
    [Parameter(Mandatory = $false, HelpMessage = "Return immediately without waiting for the start operation to complete.")]
    [switch]$Async
)
...
try {
    Write-Host "🚀 Starting VM start operation..." -ForegroundColor Green
    ...
    Write-Host "🔧 Starting VM instance '$InstanceName' in project '$ProjectId', zone '$Zone'..." -ForegroundColor Cyan
    ...
    if ($LASTEXITCODE -eq 0) {
        if ($Async) {
            Write-Host "✅ Start operation initiated for VM '$InstanceName' (async — not waiting for completion)." -ForegroundColor Green
        }
        else {
            Write-Host "✅ VM instance '$InstanceName' started successfully." -ForegroundColor Green
        }
