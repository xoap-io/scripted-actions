...
.DESCRIPTION
    This script adds outbound rules to an EC2 security group using the Grant-EC2SecurityGroupEgress cmdlet from AWS.Tools.EC2.

.PARAMETER SecurityGroupId
    The ID of the security group to add egress rules to.

.PARAMETER EgressRules
    Array of hashtables defining outbound rules. Each hashtable must contain: IpProtocol, FromPort, ToPort, CidrIp.
...
param(
    ...
    [Parameter(Mandatory = $true, HelpMessage = "Array of hashtables for outbound rules, each with keys: IpProtocol, FromPort, ToPort, CidrIp.")]
