<#
.SYNOPSIS
    Detaches a policy from a target in AWS Organizations.

.DESCRIPTION
    This script detaches a policy from a target (account, OU, or root) using the AWS CLI.
...
.EXAMPLE
    .\aws-cli-detach-policy.ps1 -PolicyId "p-12345678" -TargetId "r-1234"
...
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true, HelpMessage = "The ID of the policy to detach")]
...
try {
    $result = aws organizations detach-policy --policy-id $PolicyId --target-id $TargetId --output json 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Host "Policy detached successfully." -ForegroundColor Green
        Write-Host $result
    } else {
        Write-Error "Failed to detach policy: $result"
        exit $LASTEXITCODE
    }
} catch {
