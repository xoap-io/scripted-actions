...
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true, HelpMessage = "The ID of the route table where the route will be created")]
    [ValidatePattern('^rtb-[a-zA-Z0-9]{8,}$')]
    [string]$RouteTableId,

    [Parameter(Mandatory = $false, HelpMessage = "The IPv4 CIDR block for the route destination")]
    [ValidatePattern('^(?:\d{1,3}\.){3}\d{1,3}/\d{1,2}$')]
    [string]$DestinationCidrBlock,

    [Parameter(Mandatory = $false, HelpMessage = "The IPv6 CIDR block for the route destination")]
    [ValidatePattern('^([a-fA-F0-9:]+:+)+[a-fA-F0-9]+/\d{1,3}$')]
    [string]$DestinationIpv6CidrBlock,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of an internet gateway or VPC gateway for the route")]
    [ValidatePattern('^(igw|vgw)-[a-zA-Z0-9]{8,}$')]
    [string]$GatewayId,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of a NAT gateway for the route")]
    [ValidatePattern('^nat-[a-zA-Z0-9]{8,}$')]
    [string]$NatGatewayId,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of a network interface for the route")]
    [ValidatePattern('^eni-[a-zA-Z0-9]{8,}$')]
    [string]$NetworkInterfaceId,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of an EC2 instance for the route")]
    [ValidatePattern('^i-[a-zA-Z0-9]{8,}$')]
    [string]$InstanceId,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of a VPC peering connection for the route")]
    [ValidatePattern('^pcx-[a-zA-Z0-9]{8,}$')]
    [string]$VpcPeeringConnectionId,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of a transit gateway for the route")]
    [ValidatePattern('^tgw-[a-zA-Z0-9]{8,}$')]
    [string]$TransitGatewayId,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of a local gateway for the route")]
    [ValidatePattern('^lgw-[a-zA-Z0-9]{8,}$')]
    [string]$LocalGatewayId,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of a carrier gateway for the route")]
    [ValidatePattern('^cagw-[a-zA-Z0-9]{8,}$')]
    [string]$CarrierGatewayId,

    [Parameter(Mandatory = $false, HelpMessage = "The ID of a VPC endpoint for the route")]
    [ValidatePattern('^vpce-[a-zA-Z0-9]{8,}$')]
    [string]$VpcEndpointId,

    [Parameter(Mandatory = $false, HelpMessage = "Perform a dry run to validate parameters without creating the route")]
    [switch]$DryRun,
