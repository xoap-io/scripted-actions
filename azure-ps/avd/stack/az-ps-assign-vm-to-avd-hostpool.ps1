...
param (
    ...
    [Parameter(Mandatory=$false, HelpMessage = "The Azure region where the VM and Host Pool are located.")]
    [ValidateSet(
...
Write-Host "🚀 Assigning VM '$VmName' to Host Pool '$HostPoolName' in location '$Location'..." -ForegroundColor Green
New-AzWvdRegistrationInfo -ResourceGroupName $ResourceGroupName -HostPoolName $HostPoolName -ExpirationTime (Get-Date).AddDays(1)
...
$ScriptBlock = {
    ...
    function Remove-RdPackage($productName) {
        try {
            $product = Get-CimInstance -Query "SELECT * FROM Win32_Product WHERE Name = '$productName'"
            if ($null -ne $product) {
    ...
    function Get-InstallerFile($url, $outputPath) {
        try {
    ...
    try {
        ...
        # Download installers
        Get-InstallerFile -url $agentUrl -outputPath $agentInstallerPath
        Get-InstallerFile -url $bootLoaderUrl -outputPath $bootLoaderInstallerPath
...
try {
    ...
    $cmdRes = Invoke-AzVMRunCommand -ResourceGroupName $resourceGroupName -VMName $VmName -CommandId 'RunPowerShellScript' -ScriptString $Script.ToString() -Parameter @{'RegistrationKeyToken' = $registrationkeytoken;}
